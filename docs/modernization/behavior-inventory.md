...
**Phase:** 2 — Reload-loop implementation reconciliation
**Status:** Phase 0 inventory retained; Phase 1 lifecycle and Phase 2 reload-loop dispositions recorded
...
| BEH-004 | User / workspace | Monitoring is enabled through named requests; each project runs only while at least one request exists, and enabling triggers a complete reanalysis. | `mac/Classes/Model/Workspace.m:201-212`; `mac/Classes/Model/Project/Project.m:420-452` | v1 | `ProjectMonitor` lifecycle tests and app/UI start-stop tests | Browser-connection coupling is not retained. Phase 2 uses explicit user start/stop and does not claim an inferred initial reload or full tree scan. |
...
| BEH-006 | Filesystem / app | FSEvents paths are cached, debounced, rescanned through a tree differ, and delegated only when the calculated change set is nonempty; explicit rescan adds the root path. | `mac/FileSystemMonitoringKit/FileSystemMonitoringKit/FSMonitor.m:183-227`; callback `:246-249` | v1 | Fake-clock batching and workspace-backed FSEvents lifecycle/recovery tests | Phase 2 retains meaningful settled batches but treats FSEvents as invalidation signals. Dropped/root-change conditions enter explicit recovery; raw events are not misrepresented as a verified recursive rescan. |
...
| BEH-011 | Browser client / server | A client must send `hello` with a protocols array; protocol 7 accepts `reload` with required string `path` and optional `liveCSS`, `originalPath`, and `overrideURL`. Invalid JSON, commands, attributes, or no negotiated protocol are protocol errors. | `node_modules/livereload-protocol/lib/parser.coffee:18-80`; `:117-178` | v1 | `ReloadProtocolTests`, raw `ReloadServerTests`, and production-browser fixture | Phase 2 implements the standard monitoring subset. Saving, alert delivery, and URL override remain deferred or excluded by the feature contract. |
| BEH-012 | Browser server / user | The browser server listens, reports port occupation rather than crashing, tracks connection count, broadcasts reload messages to monitoring clients, and closes on disposal. | `node_modules/livereload-service-server/src/server.coffee:14-80`; `test/server_test.coffee:14-38` | v1 | `ReloadServerTests`, `ProjectPipelineTests`, app/UI state tests, and production-browser fixture | Phase 2 binds loopback only and isolates bounded clients. Serving historical `livereload.js`, URL override, and broad-network access remain deferred. |
...
## Phase 2 disposition (2026-07-14)

| Behavior | Modern disposition | Evidence |
|---|---|---|
| BEH-001–BEH-003 | Implemented in Phase 1; project identity, configuration, bookmark repair, and removal remain the persistence boundary. | `ProjectStoreTests.swift`, `FolderAccessTests.swift`, app tests, and foundation sprint review |
| BEH-004 | Implemented with explicit per-project start/stop/retry and in-memory runtime state. A persisted `enabled` preference does not silently restore monitoring. | `MonitoringTests.swift`, `LiveReloadAppTests.swift`, `LiveReloadAppUITests.swift` |
| BEH-005–BEH-006 | Implemented as normalized project-relative filtering plus bounded, ordered, de-duplicated settled batches. Root/loss flags stop truthful watching and require recovery. | `IgnoreRuleTests.swift`, `MonitoringTests.swift`, ADR-002 |
| BEH-007–BEH-010 | Retained for Phase 3. Phase 2 deliberately has no process execution or build-gated reload branch. | `specs/003-reload-loop/plan.md` deferred boundaries |
| BEH-011 | Implemented through bounded typed protocol-7 parsing/encoding and an owned RFC 6455 session state machine. | `ReloadProtocolTests.swift`, protocol/raw-frame fixtures, ADR-001 |
| BEH-012 | Implemented as a loopback-only listener with bounded concurrent clients, visible count/conflict state, isolated failure, restart, and manual reload. | `ReloadServerTests.swift`, `ProjectPipelineTests.swift`, app/UI tests, production-browser evidence index |

This reconciliation changes verification ownership, not the historical source evidence. Phase 2 divergences are explicit safety/scope decisions in the accepted feature contracts; they do not authorize Phase 3 build behavior or public-network serving.
