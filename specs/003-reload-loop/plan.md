...
3. **The browser endpoint has one local compatibility contract.** It binds `127.0.0.1:35729`, accepts only `/livereload`, permits absent Origin or exact loopback HTTP(S) browser origins, requires a compatible protocol-7 hello within a bounded negotiation lifetime, requires masked client frames, and bounds HTTP headers, frames, messages, and client count. It supports text, close, ping, and pong only.
